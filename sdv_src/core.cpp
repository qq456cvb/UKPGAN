...
std::vector <std::vector <float>> computeLocalDepthFeature(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
                              ...
                              float smoothing_factor)
{
   ...
   int threads_ = 1;
    ...
    auto descriptor = std::unique_ptr<Eigen::VectorXf[]>(new Eigen::VectorXf[threads_]);
